...
function draw() {
    if (data == "") return;
    // console.log("Received payload from mqtt");
...
function mouseClicked() {
    if (sq_w_size == 0 || sq_h_size == 0) return;
    if (mouseX < 0 || mouseX >= width) return;
    if (mouseY < 0 || mouseY >= height) return;

    console.log(mouseX, mouseY);
    lx = Math.floor(mouseX / sq_w_size);
    ly = Math.floor(mouseY / sq_h_size);

    console.log(lx, ly);
    payload = createPayload(lx, ly);
    sendMqttMessage(payload);
}

function createPayload(sx, sy) {
    return `{"SB_ID": "lumm_dev", "ACTION": "UPDATE_CONFIG", "PAYLOAD": {"CAP_RESOLUTION": 20, "INITIATOR_ID": "4b3f", "LIVE_FEED": true, "NAME": "sanchez", "PLUGINS": [{"INSTANCES": [{"DEBUG_OBJECTS": false, "INFERENCE_AI_ENGINE_PARAMS": {"STONES": [[${sy}, ${sx}]]}, "INSTANCE_ID": "SANCHEZ", "REQUEST": {"DATA": "OP_GET_DATA 2022-11-06 21:48:16", "TIMESTAMP": "1667764096.3427544"}, "POINTS": [0, 0, 540, 1020]}], "SIGNATURE": "SANCHEZ_01"}], "RECONNECTABLE": true, "SESSION_ID": null, "TYPE": "A_Dummy_Stream"}, "INITIATOR_ID": "4b3f"}`;
}
